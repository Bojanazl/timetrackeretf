        ...
        : AuthenticationStateProvider 
    {
        ...
        public override async Task<AuthenticationState> GetAuthenticationStateAsync()
        {
            ...
            var identity = string.IsNullOrEmpty(token)
                ? new ClaimsIdentity() : new ClaimsIdentity(GetClaimsFromTokenAndUser(token, user), "jwt");
        ...
        public async Task SetTokenAndUserAscyn(string token, UserModel user)
        {
        ...
        private IEnumerable<Claim> GetClaimsFromTokenAndUser(string token, UserModel user)
        {
            ...
            var jsonBtyes = ParseBase64WithoutPadding(payload);
            var keyValuePairs = JsonSerializer.Parse<Dictionary<string, object>>(jsonBtyes);
